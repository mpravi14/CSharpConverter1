...
if_statement 
  : 
  IF OPEN_PARENS cond=boolean_expression CLOSE_PARENS ifBdystmts=embedded_statement ( (ELSE) => ELSE elsBdy=embedded_statement )?
  ->ifStatement(condition={cond},ifBody={ifBdystmts},elseBody={elsBdy})
  ;
switch_statement 
...
foreach_statement 
	...
	->foreachStatement(variableType={varTyp},variable={$var.text},expression={expr},forEachBody={frEchBdy})
	;
jump_statement 
	: 
...
namespace_declaration 
	...
	->pkgDeclarationandClassBody(packageName={NamingUtil.toLowerCase($qi.text)},nameSpaceBody={nmsBdy},deleimiter={dl},comments={comts},className={getClassName()})
	;
...
class_body 
	...
	->classBody(classMemdeclarations={clsMemDecs},className={getClassName()})
	;
...
method_body 
	: 
	blk=method_block (comts1=comments)?
	->methodBody(body={CSharpHelper.updateRetrunStatment(blk.st.toString())},comments={comts1})
  ...
  : 
  OPEN_BRACE (comts=comments)? (lst=statement_list)? CLOSE_BRACE
  ->methodblock(comments={comts},blockStmtLst={lst})
  ;
/*
...
interface_member_declaration 
  : 
  (com=comments)? (at=attributes)? (com1=comments)? (n=NEW)? chld=interface_member_declaration_Chld 
  ->interfaceMemberDeclaration(com={com},attributes={at},com1={com1},newKwd={$n.text},child={chld})
  ;
...
:
 CLASS clsNam=IDENTIFIER -> text(value={$clsNam.text+""+assignClassName($clsNam.text)})
;
...
class_definition
  ...
  ->classDefinition(className={$clsNam.text},baseClass={basCls},classBody={clsBdy},delimiter={$dl.text},comments={comts})
  ;
struct_definition
  : 
...
method_declaration2
  ...
  ->methodDeclaration2(methodName={NamingUtil.toCamelCase($mthdNam.text)},typeParamLst={parLst},formalParamLst={fpl},paramClauses={cluses},commnents={comts},methodBody={mBdy},logMethod={CSharpHelper.updateLoggerRequest($fpl.text)})
  ;
